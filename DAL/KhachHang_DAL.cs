...
namespace DAL
{
    public class KhachHang_DAL
    {
        public static List<KhachHang_DTO> LoadKhachHang(int index)
        {
            ...
            string strTruyVan = @" SELECT * FROM KhachHang ";
        ...
        public static List<KhachHang_DTO> HienThiDanhSachCacKhachHangDaCoPhong_KH()
        {
            ...
            try
            {
                string strTruyVan = string.Format("select PDK.MaPhieuDK,KH.MaKH,KH.TenKH,KH.GioiTinh,KH.NgaySinh,KH.DiaChi,KH.CMND,SDT,KH.QuocTich,PHG.MaPhong,PHG.TenPhong from PhieuDangKy as PDK,ChiTietLoaiPhong as CTLP, KhachHang as KH,Phong as PHG where PDK.MaPhieuDK = CTLP.MaPhieuDK and PDK.MaKH = KH.MaKH and CTLP.MaLoaiPhong = PHG.MaLoaiPhong and PHG.TinhTrangPhong = 1");// and DaVaoPhong = 1
                DataTable _dt = new DataTable();
                _dt = DataProvider.fillDataTable(strTruyVan);
